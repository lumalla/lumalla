//! Smoke client: shell+shm surface, bind seat touch, log touch events (idle-waits).
...
use lumalla_wayland_protocol::protocols::wayland::{
    ...
    WL_REGISTRY_BIND_OPCODE, WL_SEAT_GET_TOUCH_OPCODE, WL_SHELL_GET_SHELL_SURFACE_OPCODE,
    WL_SHELL_SURFACE_PONG_OPCODE, WL_SHELL_SURFACE_SET_FULLSCREEN_OPCODE,
...
fn main() -> anyhow::Result<()> {
    ...
    send(
        &mut stream,
        request(seat, WL_SEAT_GET_TOUCH_OPCODE, u32_arg(13)),
    )?;

    println!(
        "wl_touch_shm ready on {}. Logging touch events; Ctrl-C to exit.",
        socket_path.display()
    );
        ...
        if event.object_id == 1 && event.opcode == 0 {
            ...
            match event.opcode {
                0 => {
                    ...
                    let surface = read_u32(&event.payload, 8)?;
                    let id = read_i32(&event.payload, 12)?;
                    let x = fixed_to_f32(read_i32(&event.payload, 16)?);
                    let y = fixed_to_f32(read_i32(&event.payload, 20)?);
                    println!(
                        "wl_touch.down serial={serial} time={time} surface={surface} id={id} ({x:.1},{y:.1})"
                    );
                }
                1 => {
                    let serial = read_u32(&event.payload, 0)?;
                    let time = read_u32(&event.payload, 4)?;
                    let id = read_i32(&event.payload, 8)?;
                    println!("wl_touch.up serial={serial} time={time} id={id}");
                }
                2 => {
                    let time = read_u32(&event.payload, 0)?;
                    let id = read_i32(&event.payload, 4)?;
                    let x = fixed_to_f32(read_i32(&event.payload, 8)?);
                    let y = fixed_to_f32(read_i32(&event.payload, 12)?);
                    println!("wl_touch.motion time={time} id={id} ({x:.1},{y:.1})");
                }
                3 => println!("wl_touch.frame"),
                4 => println!("wl_touch.cancel"),
                _ => {}
            }
        }
    }
...
fn memory_file(bytes: &[u8]) -> anyhow::Result<File> {
    let fd = unsafe { libc::memfd_create(c"lumalla-wl-touch-smoke".as_ptr(), libc::MFD_CLOEXEC) };
    if fd < 0 {
