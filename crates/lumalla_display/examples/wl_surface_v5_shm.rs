//! Smoke client for `wl_surface` v5 requests (scale, transform, damage_buffer, offset).
...
use lumalla_wayland_protocol::protocols::wayland::{
    ...
    WL_OUTPUT_TRANSFORM_NORMAL, WL_REGISTRY_BIND_OPCODE, WL_SHELL_GET_SHELL_SURFACE_OPCODE,
    ...
    WL_SURFACE_COMMIT_OPCODE, WL_SURFACE_DAMAGE_BUFFER_OPCODE, WL_SURFACE_FRAME_OPCODE,
    WL_SURFACE_OFFSET_OPCODE, WL_SURFACE_SET_BUFFER_SCALE_OPCODE,
    WL_SURFACE_SET_BUFFER_TRANSFORM_OPCODE,
};
...
fn main() -> anyhow::Result<()> {
    ...
    let pixels = solid_color([0x40, 0xb0, 0x80, 0xff]);
    ...
    send(
        &mut stream,
        request(7, WL_SURFACE_SET_BUFFER_SCALE_OPCODE, i32_arg(1)),
    )?;
    send(
        &mut stream,
        request(
            7,
            WL_SURFACE_SET_BUFFER_TRANSFORM_OPCODE,
            i32_arg(WL_OUTPUT_TRANSFORM_NORMAL as i32),
        ),
    )?;
    send(
        &mut stream,
        request(7, WL_SURFACE_OFFSET_OPCODE, {
    ...
    send(
        &mut stream,
        request(7, WL_SURFACE_DAMAGE_BUFFER_OPCODE, damage_payload),
    )?;
    send(
        &mut stream,
    ...
    println!(
        "Presented a wl_surface v5 SHM toplevel on {}. Press Ctrl-C to exit.",
        socket_path.display()
    );
    loop {
...
fn solid_color(bgra: [u8; 4]) -> Vec<u8> {
...
fn memory_file(bytes: &[u8]) -> anyhow::Result<File> {
    let fd = unsafe { libc::memfd_create(c"lumalla-wl-surface-v5".as_ptr(), libc::MFD_CLOEXEC) };
    if fd < 0 {
