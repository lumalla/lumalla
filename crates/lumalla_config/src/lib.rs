...
use log::{error, warn};
use lumalla_shared::{
    CONFIG_WATCHER_TOKEN, CallbackRef, Comms, ConfigMessage, DisplayMessage, GlobalArgs,
    InputMessage, MESSAGE_CHANNEL_TOKEN, MainMessage, MessageRunner, Mods, Output,
};
                    ...
                    CONFIG_WATCHER_TOKEN => {
                        ...
                        if let Some(config_watcher) = &self.config_watcher {
                            while let Ok(path) = config_watcher.try_recv() {
                                if let Err(err) = self.reload_config(&path) {
                                    error!("Failed to reload config from {:?}: {err}", path);
                                }
                            }
                        }
                    }
...
impl ConfigState {
    fn handle_message(&mut self, message: ConfigMessage) -> anyhow::Result<()> {
        match message {
            ...
            ConfigMessage::ForgetCallback(callback_ref) => self
                .callback_state
                .borrow_mut()
                .forget_callback(callback_ref),
            ...
            ConfigMessage::SetOnStartup(callback) => {
                if let Some(on_startup) = self.on_startup {
                    self.callback_state.borrow_mut().forget_callback(on_startup);
                }
            ...
            ConfigMessage::SetOnConnectorChange(callback) => {
                if let Some(on_connector_change) = self.on_connector_change {
                    self.callback_state
                        .borrow_mut()
                        .forget_callback(on_connector_change);
                }
    ...
    fn reload_config(&mut self, path: &std::path::Path) -> anyhow::Result<()> {
    ...
    fn run_and_watch_user_config(&mut self, args: Arc<GlobalArgs>) -> anyhow::Result<()> {
        if let Some(config_path) = &args.config {
            ...
            // Set up config file watching with mio
            let config_watcher_waker = Arc::new(Waker::new(
                self.event_loop.registry(),
                CONFIG_WATCHER_TOKEN,
            )?);
            let config_watcher =
                ConfigWatcher::new(xdg_dirs.get_config_home(), config_watcher_waker);
    ...
    fn set_default_keymaps(&mut self) -> LuaResult<()> {
        ...
        for (mods, key_name, callback) in default_keymaps {
            self.comms.input(InputMessage::Keymap {
                ...
                mods,
                callback: self.callback_state.borrow_mut().register_callback(callback),
            });
        }
...
fn init_base_module(
    ...
    module.set(
        ...
        lua.create_function(move |_, callback: LuaFunction| {
            let callback = cb_state.borrow_mut().register_callback(callback);
